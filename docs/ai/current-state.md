...
- `OrdersController`, yalnız `IOrderQueryService` üzerinden çalışan `Index` ve `Details` GET endpoint'lerini Admin ve Employee rollerine açar; mutasyon endpoint'i, `IOrderService` veya doğrudan DbContext bağımlılığı taşımaz. Sipariş NotFound güvenli HTTP 404'e, sözleşmeyle uyuşmayan tipli sonuçlar kişisel veri içermeyen structured log ve güvenli HTTP 500'e çevrilir; gerçek exception'lar merkezi hata middleware'ine yayılır.
- Sipariş liste ekranı OrderType/OrderStatus filtreleri, enum tabanlı tarih sıralaması, 10/20/50/100 sayfa boyutu ve filtre korumalı sayfalama sunar. Sale tarafı Customer, Purchase tarafı Supplier adıyla; tarih açık UTC ve tutar `tr-TR` para biçimiyle gösterilir. Detay ekranı güvenli projection üzerinden Product adı/SKU'su, miktar, birim fiyat snapshot'ı, satır toplamı ve sipariş toplamını gösterir; oluşturma, düzenleme, onaylama, iptal veya silme kontrolü üretmez.
...
- 242 xUnit testi bulunur. On iki veritabanısız `OrdersController` senaryosu sorgu/HTTP davranışını, exception yayılımını, salt-okunur action/bağımlılık sınırını ve gerçek authorization policy üzerinde Admin/Employee kabulünü kanıtlar. Yirmi yedi `SuppliersController`, beş `SupplierInputModel`, yirmi altı `CustomersController`, beş `CustomerInputModel`, otuz `ProductsController`, on dokuz `CategoriesController`, üç `HomeController`, on `TurkishDecimalModelBinder`, dört `ProductInputModel`, üç `CategoryInputModel` ve dört ilişkisel `DashboardService` testi mevcut UI/dashboard katmanlarını kapsar. Yedi `StockMovementQueryService`, sekiz `OrderQueryService`, on üç `SupplierService`, on üç `CustomerService`, on iki `ProductService`, dokuz `CategoryService`, on `OrderService`, dört saf planner, on saf ortak politika/hata sahipliği ve sekiz altyapı/Identity testi diğer mevcut davranışları doğrular; ProductService create/update testi `19.34m` fiyatını SQL Server'da aynen doğrular.
...
2 Eylül 2026 tarihinde solution derlemesi (`dotnet build StockFlow.slnx --no-restore`) sonucu:
...
| Alan | Mevcut | Hedef |
| --- | --- | --- |
| Kimlik | Cookie Identity, özel MVC login/logout, global auth fallback, seed ve Dashboard/Category/Product/Customer/Supplier/Order read rol matrisi hazır | Kalan mutasyon ve StockMovement ekranlarında rol matrisi ile role göre navigasyon |
| Veri | Domain entity/mapping/migration, Category/Product/Customer/Supplier yönetimi, sipariş mutation/query, StockMovement query ve Dashboard Service akışları hazır | Order mutation ve StockMovement Service akışlarının Controller'lara bağlanması |
| Uygulama akışı | Dashboard, Category, Product, Customer, Supplier ve salt-okunur Order akışları rol korumalı ince Controller üzerinden Service/ViewModel sınırına bağlı | Sipariş mutasyonları ile StockMovement Controller ve ekranlarının Service'lere bağlanması |
| Arayüz | StockFlow yönetim düzeni, responsive dashboard, Category/Product/Customer/Supplier CRUD ve Order liste/detay ekranları hazır | Order mutation ve StockMovement MVC ekranları; API yalnızca bonus salt-okunur kapsam |

## Bilinen riskler ve boşluklar

- Dashboard, Category, Product, Customer, Supplier ve salt-okunur Order rol matrisi controller/action ve navigasyon seviyesinde uygulanmıştır; Order mutasyonları ile StockMovement rol sınırları henüz HTTP/UI katmanına bağlanmamıştır.
...
- Sipariş sorgu Service'i salt-okunur Controller/Razor ekranlarına bağlanmıştır; sipariş mutation ve StockMovement query Service'leri henüz Controller ve yönetim ekranlarına bağlanmamıştır.
- 2 Eylül 2026 Service politika refaktörü doğrulamasında 14 saf ortak politika/planner testi, 86 odaklı Service regresyonu ve 242 testlik tam paket kullanıcıya bağlı `MSSQLLocalDB` örneğinde geçti; başarısız veya atlanan test yoktur. Çözüm ayrıca format denetiminden geçer ve 0 hata/uyarıyla derlenir.
