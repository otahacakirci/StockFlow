...
- 232 xUnit testi bulunur. On iki veritabanısız `OrdersController` senaryosu sorgu/HTTP davranışını, exception yayılımını, salt-okunur action/bağımlılık sınırını ve gerçek authorization policy üzerinde Admin/Employee kabulünü kanıtlar. Yirmi yedi `SuppliersController`, beş `SupplierInputModel`, yirmi altı `CustomersController`, beş `CustomerInputModel`, otuz `ProductsController`, on dokuz `CategoriesController`, üç `HomeController`, on `TurkishDecimalModelBinder`, dört `ProductInputModel`, üç `CategoryInputModel` ve dört ilişkisel `DashboardService` testi mevcut UI/dashboard katmanlarını kapsar. Yedi `StockMovementQueryService`, sekiz `OrderQueryService`, on üç `SupplierService`, on üç `CustomerService`, on iki `ProductService`, dokuz `CategoryService`, on `OrderService`, dört saf planner ve sekiz altyapı/Identity testi diğer mevcut davranışları doğrular; ProductService create/update testi `19.34m` fiyatını SQL Server'da aynen doğrular.
...
- 2 Eylül 2026 doğrulamasında on iki OrdersController senaryosu, sekiz ilişkisel OrderQueryService testi, on dört OrderService/planner regresyon testi ve 232 testlik tam paket kullanıcıya bağlı `MSSQLLocalDB` örneğinde geçti; başarısız veya atlanan test yoktur. Çözüm ayrıca 0 hata/uyarıyla derlenir.
