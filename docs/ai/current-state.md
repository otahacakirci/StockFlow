...
- Ortak Razor düzeni Türkçe StockFlow yönetim kimliği, rolün kullanabildiği dashboard/kategori/ürün/müşteri bağlantıları ile yalnız Admin'e gösterilen tedarikçi bağlantısını, Admin/Employee rol etiketini, kimliği doğrulanmış kullanıcı adını, TempData başarı bildirimini ve antiforgery korumalı POST logout'u sunar. Dashboard altı metriği ve son beş siparişi responsive olarak gösterir; tutarlar Türk lirası, sipariş tarihleri açık UTC biçimindedir.
...
| Alan | Mevcut | Hedef |
| --- | --- | --- |
| Kimlik | Cookie Identity, özel MVC login/logout, global auth fallback, seed ve Dashboard/Category/Product/Customer/Supplier rol matrisi hazır | Kalan iş ekranlarında rol matrisi ve role göre navigasyon |
| Veri | Domain entity/mapping/migration, Category/Product/Customer/Supplier yönetimi, sipariş mutation/query, StockMovement query ve Dashboard Service akışları hazır | Order/StockMovement Service akışlarının Controller'lara bağlanması |
| Uygulama akışı | Dashboard, Category, Product, Customer ve Supplier rol korumalı ince Controller üzerinden Service/ViewModel sınırına bağlı; diğer yönetim davranışları Service katmanında hazır | Kalan yönetim Controller ve ekranlarının Service'lere bağlanması |
| Arayüz | StockFlow yönetim düzeni, responsive dashboard ve Category/Product/Customer/Supplier CRUD ekranları hazır | Order ve StockMovement MVC ekranları; API yalnızca bonus salt-okunur kapsam |

## Bilinen riskler ve boşluklar

- Dashboard, Category, Product, Customer ve Supplier rol matrisi controller/action ve navigasyon seviyesinde uygulanmıştır; kalan iş ekranlarının rol sınırları henüz HTTP/UI katmanına bağlanmamıştır.
...
- Sipariş mutation/query ve StockMovement query Service katmanları hazır olsa da Controller, rol matrisi ve yönetim ekranlarına henüz bağlanmamıştır.
- 2 Eylül 2026 doğrulamasında Supplier Controller/ViewModel odaklı 32 veritabanısız test, on üç ilişkisel SupplierService testi ve 220 testlik tam paket kullanıcıya bağlı `MSSQLLocalDB` örneğinde geçti; başarısız veya atlanan test yoktur. Çözüm ayrıca 0 hata/uyarıyla derlenir.
