...
Depodaki uygulama erken aşamadadır. Şu anda .NET 10 MVC/Razor temeli, EF Core domain modeli, LocalDB kalıcılığı, cookie tabanlı ASP.NET Core Identity, Category/Product yönetimi ve sipariş/stok Service katmanı bulunur. Uygulama özel MVC login/logout akışı kullanır; Admin ve Employee başlangıç kullanıcıları güvenli yapılandırmadan idempotent seed edilir. `CategoryService`; doğrulamalı oluşturma/düzenleme, arama/sıralama/sayfalama, detay ve ilişki korumalı silme akışlarını yönetir. `ProductService`; kategori bilgili sorgular, doğrulamalı create/update, başlangıç stoğu ile stok korumalı düzenleme ayrımı, SKU benzersizliği ve geçmiş korumalı silmeyi yönetir. `OrderService`; Sale/Purchase Draft oluşturma ve düzenleme, sunucu fiyat snapshot'ı, toplam hesaplama, atomik confirm, cancel ve yalnız Draft silme kurallarını yönetir. Yönetim Controller ve ekranları henüz uygulanmamıştır.
...
Kırk iki xUnit testi bulunur. On iki `ProductService` ve sekiz `CategoryService` testi liste sorgularını, güvenli projection'ları, doğrulamalı create/update akışlarını, stok/audit sınırını, benzersizlik ve ilişki korumalı silme kurallarını kapsar. On `OrderService` testi Sale/Purchase Draft akışlarını, fiyat snapshot'ını, toplamı, stok değişmezliğini, iki yönlü confirm hareketlerini, yetersiz stok ve kalıcılaştırma hatasında rollback'i, cancel/silme davranışını, terminal durumları ve hata kategorilerini kapsar; kalan testler saf stok planner'ını, altyapıyı ve Identity seed davranışını doğrular. İlk test altyapısı Windows ve SQL Server LocalDB gerektirir; CI ve çapraz platform çalışması sonraki kapsamdadır.
